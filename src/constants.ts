export const INKLING_DIR = "inkling";
export const UPDATE_METADATA_PATH = `${INKLING_DIR}/.last-update.json`;
...
export const INKLING_PROVIDER_ENV_KEY = "INKLING_PROVIDER";
export const INKLING_MODEL_ID_ENV_KEY = "INKLING_MODEL_ID";
export const DEFAULT_PROVIDER = "openrouter";
export const OPENROUTER_BASE_URL = "https://openrouter.ai/api/v1";

export type InklingProvider =
  | "anthropic"
  | "baseten"
  | "fireworks"
  | "ollama"
...
export type SelectableInklingProvider = InklingProvider;
...
type ProviderConfig = {
  ...
  /**
   * When false, the provider needs no API key (e.g. a local Ollama server). The
   * credential setup flow skips the key prompt and the agent supplies a
   * placeholder key to satisfy OpenAI-compatible SDK clients.
   */
...
export const SELECTABLE_INKLING_PROVIDERS = [
  "openrouter",
  "ollama",
  "baseten",
  "fireworks",
  ...
  "openai-compatible",
  "anthropic",
] as const satisfies readonly SelectableInklingProvider[];

export const PROVIDER_CONFIGS: Record<InklingProvider, ProviderConfig> = {
  baseten: {
...
export function getProviderConfig(provider: InklingProvider): ProviderConfig {
  return PROVIDER_CONFIGS[provider];
}

export function getProviderLabel(provider: InklingProvider): string {
  return getProviderConfig(provider).label;
}

export function getProviderApiKeyEnvKey(provider: InklingProvider): string {
...
export function providerRequiresBaseUrl(provider: InklingProvider): boolean {
  return getProviderConfig(provider).requiresBaseUrl === true;
}

export function providerRequiresApiKey(provider: InklingProvider): boolean {
...
export function getDefaultModelId(provider: InklingProvider): string {
  return getProviderModelOptions(provider)[0]?.id ?? DEFAULT_MODEL_ID;
}

export function normalizeProvider(
  value: string | null | undefined,
): InklingProvider | null {
...
export function isValidProvider(value: string): value is InklingProvider {
...
export function resolveConfiguredProvider(
  ...
  return (
    normalizeProvider(env[INKLING_PROVIDER_ENV_KEY]) ??
...
export const INKLING_VERSION = "0.1.2";
