...
import { createInklingThreadId, runInklingAgent } from "./agent/index.js";
import { type InklingRunEvent, type InklingRunResult } from "./agent/types.js";
import { DEFAULT_SITE_DIR, exportSite } from "./site.js";
import { formatSearchReport, searchWiki } from "./search.js";
import { formatValidateReport, validateWiki } from "./validate.js";
import {
  ...
  INKLING_PROVIDER_ENV_KEY,
  INKLING_MODEL_ID_ENV_KEY,
  OPENROUTER_API_KEY_ENV_KEY,
  INKLING_DIR,
  resolveConfiguredProvider,
  SELECTABLE_INKLING_PROVIDERS,
  INKLING_VERSION,
  type InklingProvider,
} from "./constants.js";
...
const INKLING_LOGO_WIDTH = Math.max(
  ...INKLING_LOGO_LINES.map((line) => line.length),
);
...
function App({ command }: AppProps) {
  ...
  const [sessionProvider, setSessionProvider] =
    useState<InklingProvider>(startupProvider);
  const [sessionModelId, setSessionModelId] = useState<string | null>(
    startupModelId,
  );
  const activeRunId = useRef(0);
  const sessionThreadId = useRef(createInklingThreadId(process.cwd()));
  ...
  const [resolvedCommand, setResolvedCommand] = useState<InklingCommand | null>(
  ...
  function submitCommandRun(
    nextCommand: Extract<InklingCommand, "init" | "update">,
    message: string | null,
  ) {
    setActiveUserMessage(message);
  ...
  function clearSession() {
    ...
    sessionThreadId.current = createInklingThreadId(process.cwd());
  ...
  async function selectModel(modelId: string) {
    await saveInklingEnv({
      [INKLING_MODEL_ID_ENV_KEY]: modelId,
    });
    setSessionModelId(modelId);
  }

  async function selectProvider(provider: InklingProvider) {
    const modelId = getDefaultModelId(provider);

    await saveInklingEnv({
      [INKLING_PROVIDER_ENV_KEY]: provider,
      [INKLING_MODEL_ID_ENV_KEY]: modelId,
    });
  ...
  useEffect(() => {
    ...
    if (!process.env[apiKeyEnvKey] && !process.stdin.isTTY) {
      setRunState({
        status: "error",
        message: `${apiKeyEnvKey} is required. Run inkling in an interactive terminal to save credentials.`,
      });
      return;
    }
    ...
    runInklingAgent(resolvedCommand, process.cwd(), {
      debug: isDebugMode(),
  ...
  if (runState.status === "init-setup-saved") {
    return (
      <Box flexDirection="column">
        ...
        <StatusLine tone="active" label="Next" value="starting inkling" />
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column">
      ...
      <Panel title="Execution Plan">
        ...
        <StatusLine tone="muted" label="Output" value={`${INKLING_DIR}/`} />
        <StatusLine
          tone="muted"
          label="Startup"
...
function ErrorDiagnosticsPanel({
  ...
  return (
    <Panel title="Error Diagnostics">
      <Text color="gray">
        INKLING_DEBUG=1 is enabled. Only allowlisted, non-secret error fields
        are shown.
      </Text>
      {diagnostics.map((diagnostic) => (
...
function Header({
  ...
  const displayModelId = sanitizeHeaderValue(
    modelId ??
      process.env[INKLING_MODEL_ID_ENV_KEY] ??
  ...
  const shouldShowLogo = showLogo && terminalColumns > INKLING_LOGO_WIDTH;
    ...
    return (
      <Box flexDirection="column" marginBottom={1}>
        <Text wrap="truncate">
          ...
          <Text bold>Inkling</Text> <Text color="gray">v{INKLING_VERSION}</Text>{" "}
    ...
    <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginBottom={1}>
        <Text>
          ...
          <Text color="gray">inkling {command}</Text>
      ...
      {runs.map((run) => (
        <Box flexDirection="column" key={run.id} marginBottom={1}>
          ...
          <Text>
            ...
            <Text color="gray">
              inkling {run.command} - {run.result.model}
            </Text>
          </Text>
          <Box flexDirection="column" marginLeft={2} marginTop={1}>
            {run.log.length > 0 ? (
...
type ChatInputProps = {
  ...
  currentProvider: InklingProvider;
  onClear: () => void;
  onCommandRun: (
    command: Extract<InklingCommand, "init" | "update">,
    message: string | null,
  ) => void;
  onModelSelect: (modelId: string) => Promise<void>;
  onProviderSelect: (provider: InklingProvider) => Promise<void>;
...
function ChatInput({
  ...
  async function selectProviderMenuOption(selectedIndex: number) {
    const provider = SELECTABLE_INKLING_PROVIDERS[selectedIndex];
...
const slashCommandOptions: SlashCommandOption[] = [
  ...
  {
    description: "Run an initial Inkling documentation pass",
    id: "init",
    label: "/init",
  },
  {
  ...
  if (menuState.kind === "provider") {
    return (
      <Box flexDirection="column" marginTop={1}>
        ...
        {SELECTABLE_INKLING_PROVIDERS.map((provider, index) => (
          <MenuRow
            description={
  ...
  if (input.startsWith("/provider")) {
    ...
    return {
      ...
      selectedIndex: clampMenuIndex(
        selectedIndex,
        SELECTABLE_INKLING_PROVIDERS.length,
      ),
    };
  }
...
function getCurrentProviderOptionIndex(
  ...
  const matchingIndex = SELECTABLE_INKLING_PROVIDERS.findIndex(
...
function appendToolStartLogItem(
  log: RunLogItem[],
  event: Extract<InklingRunEvent, { type: "tool_start" }>,
  nextLogId: React.MutableRefObject<number>,
): RunLogItem[] {
...
function completeToolLogItem(
  log: RunLogItem[],
  event: Extract<InklingRunEvent, { type: "tool_end" }>,
): RunLogItem[] {
...
function isDebugMode(): boolean {
  return process.env.INKLING_DEBUG === "1";
}

function shouldShowCredentialDiagnostics(): boolean {
  return isDebugMode() || process.env.INKLING_DEBUG_CREDENTIALS === "1";
}
...
function getErrorMessage(error: unknown): string {
  const message =
    error instanceof Error ? error.message : "Inkling agent run failed.";

  if (isOpenRouterServerError(error, message)) {
    return "OpenRouter/provider returned 500 Internal Server Error. Try retrying or switching models with /model. Run with INKLING_DEBUG=1 to show provider metadata.";
  }
...
async function runLocalCommand(
  command: Extract<CliCommand, { kind: "local" }>,
): Promise<void> {
  try {
    if (command.subcommand === "export") {
      const outDir = command.args[0] ?? DEFAULT_SITE_DIR;
      const report = await exportSite(process.cwd(), outDir);

      if (report.pages === 0) {
        process.stderr.write(
          "No wiki pages found. Run inkling --init to generate documentation first.\n",
        );
        process.exitCode = 1;
        return;
      }
      ...
      process.stdout.write(
        `Exported ${report.pages} page(s) to ${report.outDir}\n` +
          `Open ${report.outDir}/index.html in a browser, or host the folder on GitHub Pages.\n`,
      );
      process.exitCode = 0;
      return;
    }
    ...
    if (command.subcommand === "search") {
      const report = await searchWiki(command.args.join(" "), process.cwd());
      process.stdout.write(`${formatSearchReport(report)}\n`);
      process.exitCode = report.query.trim().length === 0 ? 1 : 0;
      return;
    }

    // validate
    const report = await validateWiki(process.cwd());
    process.stdout.write(`${formatValidateReport(report)}\n`);
    process.exitCode = report.issues.some((issue) => issue.severity === "error")
      ? 1
      : 0;
  } catch (error) {
...
async function runPrintCommand(
  ...
  try {
    ...
    await runInklingAgent(command.command, process.cwd(), {
      debug: isDebugMode(),
      isFollowup: command.command === "chat",
      modelId: command.modelId,
      threadId: createInklingThreadId(process.cwd()),
      userMessage: command.userMessage,
      onEvent: (event) => {
        if (event.type === "text" && event.source !== "subgraph") {
  ...
  if (
    ...
    if (!hasProviderKey) {
      return {
        kind: "error",
        exitCode: 1,
        message: `${apiKeyEnvKey} is required for non-interactive runs. Run inkling in an interactive terminal to save credentials.`,
      };
    }
  }
