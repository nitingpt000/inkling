...
import { inklingEnvPath, saveInklingEnv } from "./env.js";

export type InitSetupResult = {
  modelId: string | null;
  provider: InklingProvider | null;
  savedApiKey: boolean;
...
export function needsCredentialSetup(
  ...
  return (
    process.env[INKLING_PROVIDER_ENV_KEY] === undefined ||
    needsApiKeyStep(provider) ||
    needsBaseUrlStep(provider) ||
    (modelIdOverride === null &&
      process.env[INKLING_MODEL_ID_ENV_KEY] === undefined) ||
    process.env.LANGSMITH_API_KEY === undefined
  );
}

function needsApiKeyStep(provider: InklingProvider): boolean {
...
function needsBaseUrlStep(provider: InklingProvider): boolean {
...
function isBaseUrlConfigured(provider: InklingProvider): boolean {
...
export function InitSetup({
  ...
  const [provider, setProvider] = useState<InklingProvider>(initialProvider);
  ...
  useEffect(() => {
    ...
    if (initialStep === null) {
      onComplete({
        modelId:
          modelIdOverride ?? process.env[INKLING_MODEL_ID_ENV_KEY] ?? null,
  ...
  async function submit() {
    ...
    if (step === "provider") {
      const selectedProvider =
        SELECTABLE_INKLING_PROVIDERS[providerSelectionIndex] ??
        DEFAULT_PROVIDER;
  ...
  async function completeSetup({
    ...
    try {
      ...
      const providerEnvChanged =
        process.env[INKLING_PROVIDER_ENV_KEY] !== nextProvider;

      if (providerEnvChanged) {
        updates[INKLING_PROVIDER_ENV_KEY] = nextProvider;
      }
      ...
      if (nextModelId !== null) {
        updates[INKLING_MODEL_ID_ENV_KEY] = nextModelId;
      }
      ...
      if (nextLangSmithKey !== null) {
        ...
        if (nextLangSmithKey.length > 0) {
          updates.LANGCHAIN_PROJECT = "inkling";
          ...
          // ~/.inkling/.env and tracing silently remains enabled.
          updates.LANGCHAIN_TRACING_V2 = "false";
        }
      }

      if (Object.keys(updates).length > 0) {
        await saveInklingEnv(updates);
      }
      ...
      onError(
        saveError instanceof Error
          ? saveError.message
          : "Failed to complete Inkling credential setup.",
      );
    }
  }
    ...
    <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginBottom={1}>
        <SetupStep
          label="Provider"
          state={
            process.env[INKLING_PROVIDER_ENV_KEY]
              ? "done"
              : step === "provider"
                ? "current"
                : "pending"
        ...
        <SetupStep
          ...
          detail={
            !providerRequiresApiKey(provider)
              ? `not required for ${getProviderLabel(provider)}`
              : process.env[getProviderApiKeyEnvKey(provider)]
                ? "available from environment"
                : `save ${getProviderApiKeyEnvKey(provider)} to ${inklingEnvPath}`
          }
        />
        {providerRequiresBaseUrl(provider) ? (
          <SetupStep
            ...
            detail={
              isBaseUrlConfigured(provider)
                ? "available from environment"
                : `save ${getProviderBaseUrlEnvKey(provider)} to ${inklingEnvPath}`
            }
          />
        ) : null}
        <SetupStep
          label="Model"
          state={
            modelIdOverride || process.env[INKLING_MODEL_ID_ENV_KEY]
              ? "done"
              : step === "model"
                ? "current"
                : "pending"
        ...
        />
        <SetupStep label="Inkling" state="done" detail="agent setup" />
      </Box>
      ...
      <SetupPanel title="Prompt">
        {step ? (
          ...
          <Text>Inspecting Inkling setup...</Text>
        )}
      </SetupPanel>
      ...
      {isSaving ? (
        <SetupPanel title="Saving">
          <Text>Writing Inkling setup...</Text>
        </SetupPanel>
      ) : null}
    </Box>
  );
  ...
  if (step === "provider") {
    return (
      <Box flexDirection="column">
        ...
        {SELECTABLE_INKLING_PROVIDERS.map((providerOption, index) => (
    ...
    if (isCustomModelInput) {
      return (
        <Box flexDirection="column">
          ...
          <Text>
            <Text color="gray">$</Text> {INKLING_MODEL_ID_ENV_KEY}={" "}
            <Text color="yellow">{input}</Text>
          </Text>
          <Text color="gray">Press Enter to save it.</Text>
        </Box>
...
function getInitialStep(
  ...
  if (process.env[INKLING_PROVIDER_ENV_KEY] === undefined) {
    return "provider";
  }
...
function getProviderSetupDetail(provider: InklingProvider): string {
  if (process.env[INKLING_PROVIDER_ENV_KEY]) {
...
function getModelSetupDetail(
  ...
  if (process.env[INKLING_MODEL_ID_ENV_KEY]) {
    return process.env[INKLING_MODEL_ID_ENV_KEY] ?? "";
  }
...
function shouldStartWithCustomModelInput(provider: InklingProvider): boolean {
...
function getProviderSelectionIndex(provider: InklingProvider): number {
  const selectedIndex = SELECTABLE_INKLING_PROVIDERS.findIndex(
...
function getProviderArticle(provider: InklingProvider): "a" | "an" {
