...
export type InklingContentSnapshot = string;
...
export async function getUpdateNoopStatus(
  ...
  if (head !== lastUpdate.gitHead) {
    ...
    if (
      ...
      committedPaths.some((changedPath) => !isInklingPath(changedPath))
    ) {
...
export function shouldCheckUpdateNoop(options: InklingRunOptions): boolean {
...
/**
 * Hashes Inkling content, excluding run metadata, to detect real documentation changes.
 */
export async function createInklingContentSnapshot(
  cwd: string,
): Promise<InklingContentSnapshot> {
  const inklingDir = path.join(cwd, INKLING_DIR);
  const hash = createHash("sha256");

  await addDirectoryToSnapshot(hash, inklingDir, "");
...
/**
 * Recursively adds stable file paths and bytes to the Inkling content snapshot.
 */
...
async function createGitSummary(
  ...
  if (command === "update" && lastUpdate?.gitHead) {
    ...
    if (command === "update") {
      sections.push("No prior Inkling update timestamp was found.");
    }
...
function isInklingPath(changedPath: string): boolean {
  return (
    changedPath === INKLING_DIR || changedPath.startsWith(`${INKLING_DIR}/`)
  );
}
