...
import { loadInklingEnv, inklingEnvDir } from "../env.js";
...
export async function runInklingAgent(
  command: InklingCommand,
  cwd = process.cwd(),
  options: InklingRunOptions = {},
): Promise<InklingRunResult> {
  ...
  emitDebug(options, "env=loaded ~/.inkling/.env");
  ...
  if (command === "update" && shouldCheckUpdateNoop(options)) {
    ...
    if (noopStatus.shouldSkip) {
      const message =
        "No repository changes detected since the last Inkling update; skipping agent run.";
  ...
  try {
    return await runInklingAgentWithModelFallbacks(
      command,
      cwd,
      options,
      provider,
...
async function runInklingAgentWithModelFallbacks(
  command: InklingCommand,
  cwd: string,
  options: InklingRunOptions,
  ...
  for (const [attemptIndex, attemptModelId] of modelAttempts.entries()) {
    ...
    try {
      return await runInklingAgentCore(
        command,
        cwd,
        attemptOptions,
        provider,
  ...
  throw lastError instanceof Error
    ? lastError
    : new Error("Inkling run failed after model fallback attempts.");
}

async function runInklingAgentCore(
  command: InklingCommand,
  cwd: string,
  options: InklingRunOptions,
  provider: InklingProvider,
  ...
  const inklingSnapshotBefore =
    command === "chat" ? null : await createInklingContentSnapshot(cwd);
  emitDebug(options, "inkling.snapshot=created");
  ...
  if (
    command !== "chat" &&
    inklingSnapshotBefore !== (await createInklingContentSnapshot(cwd))
  ) {
    ...
    emitDebug(
      ...
      command === "chat"
        ...
        : "metadata=skipped inkling=unchanged",
    );
  }
...
const checkpointPath = path.join(inklingEnvDir, "inkling.sqlite");
...
- For ls, read_file, write_file, edit_file, glob, and grep, use virtual paths such as /README.md, /agent/agents/main.py, and /inkling/quickstart.md.
...
async function createCheckpointer(): Promise<SqliteSaver> {
  await mkdir(inklingEnvDir, {
    recursive: true,
    mode: 0o700,
  });
  await chmodIfExists(inklingEnvDir, 0o700);
...
export function createInklingThreadId(cwd = process.cwd()): string {
...
function createThreadId(cwd: string, runId: string): string {
  ...
  return `inkling-${digest.slice(0, 32)}-${runId}`;
}
...
function emitDebug(options: InklingRunOptions, message: string): void {
  if (!options.debug) {
    return;
  }
...
function ensureProviderKey(provider: InklingProvider): void {
  ...
  if (!process.env[apiKeyEnvKey]) {
    throw new Error(
      `${apiKeyEnvKey} is required to run Inkling with ${getProviderLabel(provider)}.`,
    );
  }
}

function ensureProviderBaseUrl(provider: InklingProvider): void {
  ...
  if (!resolveProviderBaseUrl(provider)) {
    ...
    throw new Error(
      `${baseUrlEnvKey} is required to run Inkling with ${getProviderLabel(provider)}.`,
    );
  }
}
...
function resolveModelId(
  ...
  if (!isValidModelId(modelId)) {
    throw new Error(
      `Invalid model ID configured in ${INKLING_MODEL_ID_ENV_KEY}.`,
    );
  }
...
async function createModel(provider: InklingProvider, modelId: string) {
  ...
  const baseURL = resolveProviderBaseUrl(provider);
  // Keyless providers (e.g. a local Ollama server) still need a non-empty API
  // key to satisfy the OpenAI-compatible SDK client; the value is ignored.
  const apiKey =
    process.env[getProviderApiKeyEnvKey(provider)] ??
    (providerRequiresApiKey(provider) ? undefined : "not-needed");
...
function parseStreamEvent(chunk: unknown): InklingRunEvent | null {
...
function parseToolStreamEvent(payload: unknown): InklingRunEvent | null {
  if (!isRecord(payload)) {
    return null;
  }
...
function formatDebugValue(key: string, value: string | undefined): string {
  ...
  if (key === INKLING_MODEL_ID_ENV_KEY || key === INKLING_PROVIDER_ENV_KEY) {
