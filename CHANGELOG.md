...
- Updated the README banner image to the current CLI screenshot and referenced
  it by absolute URL so it renders on the npm package page.

## [0.1.1] - 2026-07-06

### Fixed

- Ensure the `inkling` bin path publishes cleanly so the global command links on
  install. Maintenance release; no functional changes to the CLI behavior.
